...
- **Doppler compensation**: No frequency offset due to satellite motion. In a real LEO NTN deployment, Doppler can be large (like tens of kHz at Ka-band).
...
- In the active Band 78 demo, the RFsim channel model is ideal (no noise injection). This is conservative, it validates the protocol stack under best-case RF conditions, which is appropriate for control-plane correctness testing.
